...
def predict_defect(image):
    ...
    if model is None:
        return "Model not loaded", 0, {c:0 for c in CLASS_NAMES}
    
    processed_image = preprocess_image(image)
    if processed_image is None:
        return "Image processing failed", 0, {c:0 for c in CLASS_NAMES}
    ...
    try:
        ...
        return "Prediction failed", 0, {c:0 for c in CLASS_NAMES}
...
with gr.Blocks() as demo:
    ...
    # Removed examples section since we don't have the files
    # You can add this back once you upload example images to your repo
